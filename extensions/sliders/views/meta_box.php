...
<div class="slider">
	<p><?php esc_html_e("Slider type", 'cardealer') ?>:</p>
	<div class="sel">
	...
	<div class="native_sliders_groups" style="display:<?php echo $page_slider_type === 'flex' ? 'block' : 'none'; ?>">
		<p><?php esc_html_e("Slider group", 'cardealer') ?>:</p>
		<?php if (!empty($slides)): ?>
			<div class="sel"><select name="page_slider">
				<option value=""><?php esc_html_e("Choose slider group", 'cardealer') ?></option>
		...
		<?php else: ?>
			<p><?php esc_html_e("You still haven't created any slider group.", 'cardealer') ?><br><a href="<?php echo admin_url() ?>/post-new.php?post_type=slidergroup"><?php esc_html_e("Click here", 'cardealer') ?></a> <?php esc_html_e("to create one.", 'cardealer') ?></p>
		<?php endif; ?>
	</div>
	...
	<?php if (is_plugin_active('LayerSlider/layerslider.php') || class_exists('LayerSlider')): ?>
		<div id="layerslider_slidegroups" <?php if ($page_slider_type != 'layerslider'): ?>style="display: none;"<?php endif; ?>>
			...
			?>
			<p><?php esc_html_e("Layerslider's groups", 'cardealer') ?>:</p>
			<div class="sel">
			<select name="layerslider_slide_group">
				...
				<?php if (!empty($sliders)) : ?>
					...
					<?php endforeach; ?>
				<?php else: ?>
					<?php esc_html_e("You still haven't created any slider group for your Layerslider.", 'cardealer') ?>
				<?php endif; ?>
			</select>
			</div>
		</div>
	...
	<?php if (function_exists('cuteslider_init')): ?>
		<div id="cutelider_slidegroups" <?php if ($page_slider_type != 'cuteslider'): ?>style="display: none;"<?php endif; ?>>
			...
			$sliders = $wpdb->get_results("SELECT * FROM $table_name
											...
											ORDER BY id ASC LIMIT 200");
			?>
			<p><?php esc_html_e("Cuteslider's groups", 'cardealer') ?>:</p>
			<div class="sel">
				<select name="cuteslider_slide_group">
					...
					<?php if (!empty($sliders)) : ?>
						...
						<?php endforeach; ?>
					<?php else: ?>
						<?php esc_html_e("You still haven't created any slider group for your Cuteslider.", 'cardealer') ?>
					<?php endif; ?>
				</select>
			</div>
		</div>
