...
<?php if (!empty($roles) && count($roles) > 1): ?>
	...
	<section class="pricing-table row">
		...
		<?php foreach ($roles as $key => $role) : ?>
			...
			<div class="pricing-item<?php if ($is_current_role): ?> featured<?php endif; ?>">
				...
				<ul class="features">
					<li><span><?php esc_html_e('Max Cars', 'cardealer'); ?>: <?php echo $role['max_cars']; ?></span></li>
					<li><span><?php esc_html_e('Disk Storage', 'cardealer'); ?>: <?php echo $role['max_images_size']; ?> <?php esc_html_e('MB', 'cardealer'); ?></span></li>
					<li><span><?php esc_html_e('Featured Cars', 'cardealer'); ?>: <?php echo $role['features_cars_count']; ?></span></li>
					<li><span><?php esc_html_e('Account Life Time', 'cardealer'); ?>: <?php echo($role['life_period'] > 0 ? $role['life_period'] : '&#8734;'); ?> <?php esc_html_e('days', 'cardealer'); ?></span>
					</li>
				</ul>
				<!-- .features -->
				...
				<footer class="footer">
					...
					if ($is_free) {
					?>
						<h3><?php esc_html_e('Free Plan', 'cardealer'); ?></h3>
					<?php
...
<?php if (!empty($features_packets)): ?>
	<div id="featured_block">
		...
		<h3 class="section-title"><?php esc_html_e('Featured Cars Bundles', 'cardealer'); ?></h3>

		<section class="pricing-table row">
			<?php foreach ($features_packets as $key => $packet) : ?>

				<div class="pricing-item<?php if ($packet['featured'] == 1): ?> featured<?php endif; ?>">
						...
						<dl>
							...
							<?php foreach ($split_price as $k => $text) : ?>
								<dd>
									...
									<?php else: ?>
										<span data-month="" class="sup <?php if (strlen($text) == 3): ?>size-medium<?php endif; ?>"><?php echo $text ?></span>
									<?php endif; ?>
								</dd>
							<?php endforeach; ?>
					...
					<ul class="features">
						<li><span><?php esc_html_e('Count', 'cardealer'); ?>: <?php echo $packet['count']; ?></span></li>
						<li><span><?php esc_html_e('Life Time', 'cardealer'); ?>: <?php echo($packet['life_period'] > 0 ? $packet['life_period'] : '&#8734;'); ?> <?php esc_html_e('days', 'cardealer'); ?></span></li>
					</ul>
					<!-- .features -->
