...
//Custom page navigation
	...
	public static function get_monts_names($num) {
		$monthes = array(
			0 => esc_html__('January', 'cardealer'),
			1 => esc_html__('February', 'cardealer'),
			2 => esc_html__('March', 'cardealer'),
			3 => esc_html__('April', 'cardealer'),
			4 => esc_html__('May', 'cardealer'),
			5 => esc_html__('June', 'cardealer'),
			6 => esc_html__('July', 'cardealer'),
			7 => esc_html__('August', 'cardealer'),
			8 => esc_html__('September', 'cardealer'),
			9 => esc_html__('October', 'cardealer'),
			10 => esc_html__('November', 'cardealer'),
			11 => esc_html__('December', 'cardealer'),
		);
	...
	public static function get_short_monts_names($num) {
		$monthes = array(
			0 => esc_html__('jan', 'cardealer'),
			1 => esc_html__('feb', 'cardealer'),
			2 => esc_html__('mar', 'cardealer'),
			3 => esc_html__('apr', 'cardealer'),
			4 => esc_html__('may', 'cardealer'),
			5 => esc_html__('jun', 'cardealer'),
			6 => esc_html__('jul', 'cardealer'),
			7 => esc_html__('aug', 'cardealer'),
			8 => esc_html__('sep', 'cardealer'),
			9 => esc_html__('oct', 'cardealer'),
			10 => esc_html__('nov', 'cardealer'),
			11 => esc_html__('dec', 'cardealer'),
		);
	...
	public static function get_days_of_week($num) {
		$days = array(
			0 => esc_html__('Sunday', 'cardealer'),
			1 => esc_html__('Monday', 'cardealer'),
			2 => esc_html__('Tuesday', 'cardealer'),
			3 => esc_html__('Wednesday', 'cardealer'),
			4 => esc_html__('Thursday', 'cardealer'),
			5 => esc_html__('Friday', 'cardealer'),
			6 => esc_html__('Saturday', 'cardealer'),
		);
	...
	public static function get_post_categories() {
		...
		$post_categories[0] = esc_html__('All Categories', 'cardealer');
	...
	public static function draw_breadcrumbs() {
		...
		if ( is_single() || is_page() || is_archive() || $is_page_for_posts || is_search() ) {
			...
			if (is_single()) {
				...
				if (!empty($categories)) {
					...
					$breadcrumbs[] = array(
						'href' => $cat_url,
						'text' => $text,
						'title' => sprintf( esc_html__("View all posts in %s", 'cardealer'), $text),
					);
				}
			...
			if (is_search()) {
				$breadcrumbs[] = array(
					'href' => '',
					'text' => esc_html__('Search: ', 'cardealer') . get_search_query(),
					'title' => '',
				);
			}
	...
	public static function get_the_category_list($separator = '', $parents = '', $post_id = false) {
		...
		if (empty($categories))
			return apply_filters('the_category', esc_html__('Uncategorized', 'cardealer'), $separator, $parents);
		...
		foreach ($categories as $category) {
			...
			if ($cat == $category->term_id) {
				...
				$thelist .= '<a href="' . esc_url(get_category_link($category->term_id)) . '" title="' . sprintf( esc_html__("View all posts in %s", 'cardealer'), $category->name) . '" ' . $rel . '>' . $category->name . '</a></li>';
			}
		}
